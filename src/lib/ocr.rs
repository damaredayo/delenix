...
mod ocr {
    use image::{EncodableLayout, ImageFormat, ImageBuffer, Luma, Pixel};
    use std::{io::Cursor, collections::HashMap};
    use tesseract::Tesseract;

    pub fn ocr(tessdata_path: &Option<String>, img: &[u8]) -> Result<String, Box<dyn std::error::Error>> {
        ...
        let image = image::load(Cursor::new(img), img_format).unwrap();
        ...
        grayscale_image.write_to(&mut tiff_buffer, ImageFormat::Tiff).unwrap();
...
mod ocr {
    pub fn ocr(_: &Option<String>, _: &[u8]) -> Result<String, Box<dyn std::error::Error>> {
        println!("Tesseract not enabled. Please use the `tesseract` feature to enable it.");
        Ok("".to_string())
    }
}
